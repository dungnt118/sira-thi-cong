...
| GAP-03 | Delivery | Chưa có `Task module` xuyên vai trò cho PM/Supervisor/Worker | Code có checklist thi công nhưng không có task orchestration | Không kiểm soát được người làm, người duyệt, deadline, phụ thuộc | Critical | Tạo module `Project Task / Work Package` |
...
| GAP-08 | Role model | Mô hình vai trò đang mâu thuẫn giữa V2, V3 và code | V2 có 6 vai trò, V3 gom 4, code lại song song supervisor/worker | Sai phân quyền và UX | Critical | Chốt role model V4 và mapping transition |
...
| GAP-17 | Warranty | Bảo hành, bảo dưỡng, lịch nhắc mới dừng ở mô tả/wireframe | Route hiện tại còn `ComingSoon` | Không đóng vòng đời dịch vụ | High | Chuẩn hóa `Warranty Card`, `Maintenance Visit` |
...
| GAP-21 | File governance | Evidence/file chưa có version, retention, storage strategy và quyền truy cập chi tiết | BA-V2 có nói, code chưa có | Rủi ro lưu trữ và pháp lý | Medium | Thiết kế `FileAsset` chuẩn |
...
- tự động sinh task nào cho PM/Supervisor/Worker

Đây là lý do V4 bắt buộc thêm `Stage Playbook`.

### 3.2 Task management cho PM, Supervisor, Worker
