...
|---|---|---|---|---|---|
| GS-01 | Trang chủ hiện trường | Xem việc hôm nay, việc trễ, cảnh báo hiện trường | `/gs/home` -> `src/pages/kỹ thuật/Kỹ thuậtHome.tsx` | Đã có seed | Critical |
| GS-02 | Danh sách công trình phụ trách | Xem các công trình/visit được giao | `/gs/projects` -> đang dùng lại `Kỹ thuậtHome` | Có một phần | Critical |
...
| GS-06 | Checklist bước thi công | Theo dõi và cập nhật trạng thái từng bước | `/gs/checklist/:id` -> `src/pages/kỹ thuật/Checklist.tsx` | Đã có seed | Critical |
| GS-07 | Upload ảnh/video minh chứng | Chụp/tải file chứng cứ cho từng bước | `/gs/evidence/:projectId/:stepId` -> `src/pages/kỹ thuật/EvidenceUpload.tsx` | Đã có seed | Critical |
...
| GS-11 | Ký nhận vật tư | Xác nhận nhận vật tư và tình trạng hàng hóa | `/gs/materials` -> `ComingSoon` | Có một phần | Critical |
...
| GS-13 | Báo cáo sự cố | Tạo case sự cố và escalate | `/gs/incident` -> `src/pages/kỹ thuật/IncidentReport.tsx` | Đã có seed | High |
...
| GS-23 | Hồ sơ Giám sát | Xem thông tin cá nhân, phạm vi phụ trách | `/gs/profile` -> `ComingSoon` | Có một phần | Medium |
