...
const ContractDetail: React.FC = () => {
    ...
    const renderLinkedProjects = () => {
        const projectColumns = [
            { title: 'Mã DA', dataIndex: 'code', key: 'code', render: (c: string, rec: typeof mockLinkedProjects[0]) => <a onClick={() => navigate(`/admin/ql/projects/${rec.id}`)}>{c}</a> },
                    ...
                    columns={[
                        {
                            ...
                            render: (c: string, rec: typeof mockMaintenanceRequests[0]) => (
                                <a onClick={() => navigate(`/admin/ql/contracts/${_id}/maintenance/${rec.key}`)} style={{ fontWeight: 600 }}>{c}</a>
                            ),
                        },
                            ...
                            render: (_: any, rec: typeof mockMaintenanceRequests[0]) => (
                                <Space size="small">
                                    <Button size="small" icon={<DollarOutlined />}
                                        onClick={() => navigate(`/admin/ql/contracts/${_id}/maintenance/${rec.key}`)}>Thêm CP</Button>
                                    <Button size="small" type="link"
                                        onClick={() => navigate(`/admin/ql/contracts/${_id}/maintenance/${rec.key}`)}>Xem</Button>
                                </Space>
                            ),
                        },
                    ]}
    ...
    return (
        <div>
            ...
            <Row align="middle" justify="space-between" style={{ marginBottom: 24 }}>
                <Space wrap>
                    <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/admin/ql/contracts/all')}>
                        {!isMobile && 'Quay lại'}
                    </Button>
                ...
                <Space wrap>
                    ...
                    {isDraft ? (
                        <Button type="primary" icon={<EditOutlined />}
                            onClick={() => navigate(`/admin/ql/contracts/edit/${_id}`)}>
                            {!isMobile && 'Chỉnh sửa'}
                        </Button>
                    ) : (
