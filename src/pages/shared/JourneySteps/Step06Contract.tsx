...
export const Step06Contract: React.FC<Step06ContractProps> = ({ journeyId, isEditable = false, onSave }) => {
    ...
    if (!isEditable) {
        return (
            <Card title="Chi tiết bước: Làm hợp đồng (Sale, PM)" bordered={false} className="ky-card">
                <Result
                    status="info"
                    title="Làm hợp đồng"
                    subTitle="Thông tin chi tiết của bước Làm hợp đồng ở chế độ xem (Readonly)."
                />
            </Card>
        );
    }
    ...
    return (
        <Card title="Thực hiện: Làm hợp đồng" bordered={false} className="ky-card">
