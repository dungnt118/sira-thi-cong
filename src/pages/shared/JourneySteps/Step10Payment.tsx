...
export const Step10Payment: React.FC<Step10PaymentProps> = ({ isEditable = false, onSave }) => {
    ...
    if (!isEditable) {
        return (
            <Card title="Chi tiết bước: Thanh Toán (Kế toán)" bordered={false} className="ky-card">
                <Result
                    status="info"
                    title="Thanh Toán"
                    subTitle="Thông tin chi tiết của bước Thanh Toán ở chế độ xem (Readonly)."
                />
            </Card>
        );
    }
    ...
    return (
        <Card title="Thực hiện: Thanh Toán" bordered={false} className="ky-card">
