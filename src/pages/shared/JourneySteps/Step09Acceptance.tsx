...
export const Step09Acceptance: React.FC<Step09AcceptanceProps> = ({ journeyId, isEditable = false, onSave }) => {
    ...
    if (!isEditable) {
        return (
            <Card title="Chi tiết bước: Nghiệm Thu (Giám sát, Kỹ thuật)" bordered={false} className="ky-card">
                <Result
                    status="info"
                    title="Nghiệm Thu"
                    subTitle="Thông tin chi tiết của bước Nghiệm Thu ở chế độ xem (Readonly)."
                />
            </Card>
        );
    }
    ...
    return (
        <Card title="Thực hiện: Nghiệm Thu" bordered={false} className="ky-card">
