...
export const Step07Advance: React.FC<Step07AdvanceProps> = ({ journeyId, isEditable = false, onSave }) => {
    ...
    if (!isEditable) {
        return (
            <Card title="Chi tiết bước: Tạm ứng (Kế toán)" bordered={false} className="ky-card">
                <Result
                    status="info"
                    title="Tạm ứng"
                    subTitle="Thông tin chi tiết của bước Tạm ứng ở chế độ xem (Readonly)."
                />
            </Card>
        );
    }
    ...
    return (
        <Card title="Thực hiện: Tạm ứng" bordered={false} className="ky-card">
