...
export const quotationService = {
  ...
  /** Lưu nhiều Quotation — phải dùng save_many_content (mảng), không dùng save_content (một Dictionary). */
  async saveManyQuotations(data: any[]): Promise<any[]> {
    ...
    if (response?.code != null && response.code !== 0 && response.code !== 202) {
      throw new Error(response?.message || 'Không thể lưu hàng loạt Quotation');
    }
    const savedData = response?.data;
    if (savedData == null) {
      throw new Error(response?.message || 'Không thể lưu hàng loạt Quotation (thiếu dữ liệu trả về)');
    }
